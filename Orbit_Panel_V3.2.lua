...
local Gui=New("ScreenGui",{Name="OrbitPanelV32",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling},PlayerGui)
...
New("TextLabel",{Size=UDim2.new(1,-110,1,0),Position=UDim2.fromOffset(16,0),BackgroundTransparency=1,Text="🌌 ORBIT PANEL V3.2",TextColor3=Color3.fromRGB(245,240,255),TextSize=18,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6},Top)
...
local function InfoPage()
    ContentTitle.Text="ℹ️ Information";Clear();Label("🌌 ORBIT PANEL V3.2",42);Label("Left-side feature navigation with a right-side content/settings panel.",48);Label("V3.2: cleaner selection states, live orbit status, optional auto-stop, hover feedback, and improved mobile interaction.",55);Label("Orbit Presets, Roles, and Nurse were removed from V3.1.",45)
end
local function CreditsPage()
    ContentTitle.Text="💳 Credits";Clear();Label("🌌 Orbit Panel V3.2",40);Label("Created & Developed by Franz 🔥",40);Label("UI Design\nDesigned by Franz",45);Label("Scripting & Development\nBuilt with Roblox Lua",48);Label("Special Thanks\nThanks to everyone who tested Orbit Panel and gave feedback 🤝",58);Label("Version\nOrbit Panel V3.2\nBuilt from an idea into reality.",60)
end
...
local function Select(name)
    State.Feature=name
    for n,b in pairs(FeatureButtons) do
        local selected = (n == name)
        b.BackgroundColor3 = selected and State.Accent or Color3.fromRGB(35,27,53)
        b.TextColor3 = selected and Color3.new(1,1,1) or Color3.fromRGB(235,230,245)
        local st = FeatureStrokes[n]
        if st then
            st.Color = selected and Color3.new(1,1,1) or State.OutlineColor
            st.Transparency = selected and .05 or .65
...
for i,name in ipairs(Names) do
    local b=New("TextButton",{Size=UDim2.new(1,-8,0,40),BackgroundColor3=Color3.fromRGB(35,27,53),BorderSizePixel=0,Text=name,TextColor3=Color3.fromRGB(235,230,245),TextSize=13,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=i,ZIndex=5},FeatureScroll)
    Corner(b,8)
    New("UIPadding",{PaddingLeft=UDim.new(0,10)},b)
    FeatureStrokes[name]=Stroke(b,State.OutlineColor,1,.65)
    b.MouseEnter:Connect(function() if State.Feature~=name then Tween(b,.12,{BackgroundColor3=Color3.fromRGB(45,35,65)}) end end)
    b.MouseLeave:Connect(function() if State.Feature~=name then Tween(b,.12,{BackgroundColor3=Color3.fromRGB(35,27,53)}) end end)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if State.AutoOrbitStop and h and h.WalkSpeed>=22 then State.Orbit=false end
    if State.Orbit then
